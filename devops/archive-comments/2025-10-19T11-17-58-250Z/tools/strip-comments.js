...
function shouldProcess(file) {
  ...
  const ext = path.extname(file) || path.basename(file); // Dockerfile has no ext
...
function stripComments(content, file) {
  ...
  let out = content;
  // Remove block comments for C-like syntaxes
  out = out.replace(/\/\*[\s\S]*?\*\//g, '');
  // Remove single-line comments //
  out = out.replace(/(^|[^:]|^)\/\/.*$/gm, '');
  // Remove Python/Shebang comments starting with #
